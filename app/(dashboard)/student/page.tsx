...
export default async function StudentPage({ searchParams }: { searchParams: Record<string, string | string[] | undefined> }) {
    ...
    if (courseIdsForSurveys.length) {
      ...
      if (surveys?.length) {
        ...
        (bookings || []).forEach((b) => {
          ...
          if (!survey) return;
          const startVal =
            (b as any).course_start
            || (Array.isArray((b as any).course_dates) ? (b as any).course_dates[0]?.start_date : (b as any).course_dates?.start_date);
  ...
  return (
    <div className="space-y-8">
      <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4 bg-gradient-to-br from-slate-950/90 via-slate-900/85 to-slate-950/90 border border-white/12 rounded-2xl p-4 sm:p-6 shadow-2xl relative overflow-hidden">
