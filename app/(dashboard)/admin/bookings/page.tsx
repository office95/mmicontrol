...
export default function BookingsPage() {
  ...
  const deleteBooking = async (id: string) => {
    if (!id) return;
    const ok = confirm('Buchung und alle zugehörigen Zahlungen endgültig löschen?');
    if (!ok) return;
    ...
    <div className="space-y-6">
            ...
            {!loading && filtered.length > 0 && (
              <table className="table-modern">
                ...
                <tbody>
                  {filtered.map((b) => (
                    <tr key={b.id}>
                      <td className="whitespace-nowrap">{b.booking_date ? new Date(b.booking_date).toLocaleDateString() : '—'}</td>
                      <td>{b.student_name ?? '—'}</td>
                      <td>{b.course_title ?? '—'}</td>
                      <td className="whitespace-nowrap">{b.course_start ? new Date(b.course_start).toLocaleDateString() : '—'}</td>
                      ...
                      <td className="text-slate-800">{b.partner_name ?? '—'}</td>
                      <td>
      ...
      {selected && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 px-4">
          <div className="w-full max-w-6xl rounded-2xl bg-white text-ink shadow-2xl p-6 relative max-h-[90vh] overflow-y-auto">
            <button
            ...
            <div className="flex flex-col gap-2 md:flex-row md:items-start md:justify-between mb-4 pr-10">
              <div>
                <h3 className="text-2xl font-semibold">Buchung verwalten</h3>
                <p className="text-sm text-slate-600">
                  {selected.student_name ?? '—'} · {selected.course_title ?? '—'} · Code {selected.booking_code ?? '—'}
                </p>
              </div>
              <button
                className="inline-flex items-center gap-2 rounded-lg bg-rose-600 text-white px-3 py-2 text-sm font-semibold shadow hover:bg-rose-700 border border-rose-700"
